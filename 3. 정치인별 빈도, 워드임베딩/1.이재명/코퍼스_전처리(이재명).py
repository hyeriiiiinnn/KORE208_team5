...
# 2. JSON 로딩
with open("이재명.json", "r", encoding="utf-8") as f:
    data = json.load(f)
articles = data["이재명"]  # 리스트
...
kiwi = Kiwi()
user_words = ['개혁신당', '더불어민주당', '국민의힘', '최고득표율',
               '이 후보', '인공지능',"직권남용",'양자대결',
               "김 후보","출구조사","선거법","더중플",
              "권영국","어대명","방사능","당시엔"]
...
# 8. CSV 저장
df.to_csv("이재명_tagged_filtered_ver2.csv", index=False, encoding="utf-8-sig")
